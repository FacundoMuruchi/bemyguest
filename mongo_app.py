...
from services import multiengine_controller as controller
...
def import_dataset(dataset_path=DEFAULT_DATASET_PATH):
    return controller.import_dataset_to_all(dataset_path, reset_first=True)
...
def document_label(collection_name, document):
    if collection_name == "Usuarios":
        return user_label(document)
    if collection_name == "Hoteles":
        return hotel_label(document)
    if collection_name == "Habitaciones":
        return room_label(document)
    if collection_name == "Reservas":
        return (
            f"Reserva {document.get('_id')} | usuario {document.get('usuario_id')} | "
            f"habitación {document.get('habitacion_id')} | {document.get('estado')}"
        )
    if collection_name == "Reseñas":
        return f"Reseña {document.get('_id')} | hotel {document.get('hotel_id')} | usuario {document.get('usuario_id')}"
    return str(document.get("_id", document))
...
def show_operation_result(result):
    st.session_state["last_success"] = result.mongo_detail
    st.session_state["last_output"] = result.as_text()
    if result.warnings:
        st.session_state["last_warning"] = "La operación en Mongo se completó, pero hubo motores secundarios con errores."
    st.rerun()


def save_document(collection_name, document):
    create_actions = {
        "Usuarios": controller.create_usuario,
        "Hoteles": controller.create_hotel,
        "Habitaciones": controller.create_habitacion,
        "Reservas": controller.create_reserva,
        "Reseñas": controller.create_resena,
    }
    result = create_actions[collection_name](document)
    ...
    with st.form("create_user_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Registrar usuario", type="primary"):
            ...
            if document is not None:
                save_document("Usuarios", document)
...
def show_hotel_form():
    with st.form("create_hotel_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Registrar hotel", type="primary"):
            ...
            document = merge_extra_attributes(document, raw_extra_attributes)
            if document is not None:
                save_document("Hoteles", document)
...
def show_room_form():
    ...
    with st.form("create_room_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Registrar habitación", type="primary"):
            ...
            document = merge_extra_attributes(document, raw_extra_attributes)
            if document is not None:
                save_document("Habitaciones", document)
...
def show_booking_form():
    ...
    with st.form("create_booking_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Registrar reserva", type="primary"):
            ...
            document = merge_extra_attributes(document, raw_extra_attributes)
            if document is not None:
                save_document("Reservas", document)
...
def show_review_form():
    ...
    with st.form("create_review_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Registrar reseña", type="primary"):
            ...
            document = merge_extra_attributes(document, raw_extra_attributes)
            if document is not None:
                save_document("Reseñas", document)
...
def show_individual_delete():
    st.subheader("Eliminar documento individual")
    collection_name = st.selectbox(
        "Colección",
        list(COLLECTIONS.keys()),
        key="delete_collection_name",
    )
    docs = load_docs(COLLECTIONS[collection_name])
    if not docs:
        st.info("No hay documentos para eliminar en esta colección.")
        return

    selected_doc = st.selectbox(
        "Documento",
        docs,
        format_func=lambda doc: document_label(collection_name, doc),
    )
    with st.expander("Ver documento seleccionado"):
        st.json(serialize_value(selected_doc))

    confirm = st.checkbox("Confirmo que quiero eliminar este documento y sincronizar los motores")
    if st.button("Eliminar documento", type="primary", disabled=not confirm):
        result = controller.delete_document(collection_name, selected_doc)
        show_operation_result(result)


def show_collection_delete():
    st.subheader("Eliminar colecciones completas")
    ...
    if st.button("Eliminar colecciones", disabled=not selected or not confirm):
        outputs = []
        warnings = False
        for name in selected:
            result = controller.delete_collection(name)
            outputs.append(result.as_text())
            warnings = warnings or bool(result.warnings)
        st.session_state["last_success"] = "Colecciones eliminadas."
        st.session_state["last_output"] = "\n\n".join(outputs)
        if warnings:
            st.session_state["last_warning"] = "Mongo eliminó correctamente, pero hubo motores secundarios con errores."
        st.rerun()


def main():
    st.set_page_config(page_title="BeMyGuest - MongoDB", layout="wide", page_icon="📝")
    ...
    if "last_warning" in st.session_state:
        st.warning(st.session_state.pop("last_warning"))
    ...
    with seed_tab:
        ...
        if st.button("Importar dataset", type="primary"):
            try:
                ...
                st.session_state["last_output"] = result.as_text()
                if result.warnings:
                    st.session_state["last_warning"] = "Mongo importó correctamente, pero hubo motores secundarios con errores."
                st.rerun()

    with admin_tab:
        st.subheader("Mantenimiento")
        delete_one_tab, delete_many_tab = st.tabs(["Documento individual", "Colecciones completas"])
        with delete_one_tab:
            show_individual_delete()
        with delete_many_tab:
